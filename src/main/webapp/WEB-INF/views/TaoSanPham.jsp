...
<div class="d-flex justify-content-center mt-3">
    <div class="col-md-8 account-info" style="background-color: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1); height: 700px;">
        <h2 class="text-center">Thêm sản phẩm</h2>
        <form action="${pageContext.request.contextPath}/san-pham/save" method="post">
            <!-- Hidden field to handle the product ID for update -->
            <input type="hidden" name="id" value="${sanPham.id != null ? sanPham.id : 0}">

            <div class="mb-3">
                <label class="form-label">Danh mục sản phẩm</label>
                <select class="form-select" name="danhMuc.id">
                    <c:forEach var="dm1" items="${listDM1}">
                        <option value="${dm1.id}" ${dm1.id == sanPham.danhMuc.id ? 'selected' : ''}>
                                ${dm1.tenDanhMuc}
                        </option>
                    </c:forEach>
                </select>
            ...
            <div class="mb-3">
                ...
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="trangThai" value="Active" ${sanPham.trangThai == 'Active' ? 'checked' : ''}>
                    <label class="form-check-label">Active</label>
                </div>
                <div class="form-check">
                    <input class="form-check-input" type="radio" name="trangThai" value="UnActive" ${sanPham.trangThai == 'UnActive' ? 'checked' : ''}>
    ...
    <table class="table" style="background-color: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.1); height: 400px;">
        ...
        <c:forEach items="${listSP}" var="sp">
            <tr>
                ...
                <td>${sp.ngaySua}</td>
                <td>
                    <a href="${pageContext.request.contextPath}/san-pham/edit/${sp.id}" class="btn btn-primary btn-sm">Sửa</a>
                    <a href="${pageContext.request.contextPath}/san-pham/delete/${sp.id}" class="btn btn-danger btn-sm" onclick="return confirm('Bạn có chắc muốn xóa danh mục này?');">Xóa</a>
                </td>
            </tr>
        </c:forEach>
        </tbody>
